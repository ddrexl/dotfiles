...
-- Change Working Directory to that of the current file. The vimrc used
-- `cmap`, and mode 'ca' here is a cabbrev -- both fire anywhere in any
-- command line, including the '/' and '?' search lines, so plain
-- `:cabbrev cwd lcd %:p:h` turned a `/cwd` search into E486. Expand only
-- when the whole ':' command line is the word itself.
local function cmd_alias(lhs, rhs)
    map("ca", lhs, function()
        local at_start = vim.fn.getcmdtype() == ":" and vim.fn.getcmdline() == lhs
        return at_start and rhs or lhs
    end, { expr = true })
end
cmd_alias("cwd", "lcd %:p:h")
cmd_alias("cd.", "lcd %:p:h")
